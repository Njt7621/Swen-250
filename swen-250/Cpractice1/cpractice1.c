...
int find_next_word( char mystring[], int start_index )
{
   ...
   while (mystring[next_index] != '\0' && mystring[next_index] != ' ') {
       	next_index++;
   }
   return skip_spaces(mystring, next_index);
}
...
int change_to_upper( char mystring[], int index )
{
    if (mystring[index] >= 97 && mystring[index] <= 122) {
        mystring[index] = mystring[index] - 'a' + 'A';
	return 1;
    }
    return 0;
}
...
{
    int index = skip_spaces(mystring, 0);
    int count = 0;
    while (index != -1) {
