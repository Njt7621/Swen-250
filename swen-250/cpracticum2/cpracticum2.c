...
unsigned char my_random()
{
	...
	return mylist[index -1];	// Fix this -- it is incorrect but allows the program to compile and run.
}
...
//ote that you can assume that the p_first and p_second arrays are the same size.
...
int *create_array_of_products( int *p_first, int *p_second, int number_of_entries )
{
	...
	int *p_result = malloc(sizeof(p_second));
	int i;
	for(i = 0; i<number_of_entries; ++i){
	p_result[i] = p_second[i] * p_first[i];
	}
	return 0;
}
